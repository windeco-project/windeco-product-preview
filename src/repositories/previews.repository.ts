...
class PreviewsRepositoryClass extends AbstractRepository<
  PreviewInterface,
  PreviewFormData
> {
  constructor() {
  ...
  create(data: PreviewFormData): Promise<PreviewInterface> {
    return this.client
      .post(this.base, this.toFormData(data))
      .then(({ data }) => data);
  }

  get(id: string): Promise<PreviewInterface> {
    return this.client
      .get(`${this.base}/${id}/details`)
      .then(({ data }) => data);
  }
  ...
  getPreviewRender(
    ...
    const params = this.encode(
      JSON.parse(JSON.stringify({ m: materials, h: hidden, e: environment }))
    );
